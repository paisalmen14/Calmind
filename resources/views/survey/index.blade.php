<x-app-layout>
  ...
  {{-- ========================================================= --}}
  {{-- BAGIAN SCRIPT YANG DIPERBAIKI TOTAL --}}
  {{-- ========================================================= --}}
  @push('scripts')
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      questions.forEach((q, index) => {
        ...
        questionBlock.innerHTML = `
                    ...
                    <div class="options mt-3 space-y-2">
                        <label class="flex items-center"><input type="radio" name="${q.id}" value="0" required class="mr-2"> 0 - Tidak pernah</label>
                        <label class="flex items-center"><input type="radio" name="${q.id}" value="1" required class="mr-2"> 1 - Kadang-kadang</label>
                        <label class="flex items-center"><input type="radio" name="${q.id}" value="2" required class="mr-2"> 2 - Sering</label>
                        <label class="flex items-center"><input type="radio" name="${q.id}" value="3" required class="mr-2"> 3 - Hampir selalu</label>
                    </div>
                `;
      ...
      surveyForm.addEventListener('submit', async function(event) {
        ...
        // --- METODE PENGUMPULAN DATA BARU YANG LEBIH ANDAL ---
        const formData = new FormData(surveyForm);
        const answers = Object.fromEntries(formData.entries());

        // Pastikan semua nilai adalah angka
        for (const key in answers) {
          answers[key] = parseInt(answers[key], 10);
        }

        if (Object.keys(answers).length < 42) {
          showError('Harap jawab semua 42 pertanyaan.');
          return;
        }
