                ...
                <form action="{{ route('daily-diary.update', $dailyDiary) }}" method="POST">
                    ...
                    <div class="p-8">
                        ...
                        <div class="mb-6">
                            ...
                            <x-text-input id="title" name="title" type="text" class="mt-1 block w-full bg-slate-50 border-gray-300 rounded-lg shadow-sm focus:border-black focus:ring-black" :value="old('title', $dailyDiary->title)" required autofocus />
                            @error('title')
                        ...
                        <div>
                            <label for="content" class="block font-gabarito text-2xl text-black">Ada cerita apa hari ini? {{ $dailyDiary->entry_date->isoFormat('dddd, D MMMM YYYY') }}?</label>
                            <p class="text-sm text-black mb-4">Disinilah tempat untukmu menceritakan semua hal yang terjadi hari ini.</p>
                            
                            <textarea name="content" id="content" rows="10" 
                                      class="mt-1 block w-full bg-slate-50 border-gray-300 rounded-lg shadow-sm focus:border-black focus:ring-black">{{ old('content', $dailyDiary->content) }}</textarea>
                            @error('content')
                                <p class="text-sm text-red-600 mt-2">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="flex items-center justify-end gap-4 px-8 py-4">
                        <a href="{{ route('daily-diary.index') }}" class="text-sm font-gabarito text-black hover:text-[#E14434]">Batal</a>
